...
class GoBoard(object):
    ...
    def store(self, code, score, move, color):
        key = (code, color)
        value = (score, move)
        self.table[key] = value
    
    # Python dictionary returns 'None' if key not found by get()
    def lookup(self, code, color):
        key = (code, color)
    ...
    def negamax(self, color):
        code = self.code()
        lookup = self.lookup(code, color)
        if lookup != None:
            return lookup[0]

        legal_moves = GoBoardUtil.generate_legal_moves(self, color)
        if len(legal_moves) == 0: # End of game
            self.store(code, -1, None, color)
            return -1  # evaluate to play
        
        for m in legal_moves:
            self.board[m] = color # play move
            value = -self.negamax(GoBoardUtil.opponent(color))
            self.board[m] = EMPTY # undo move
            if value == 1:
                self.store(code, 1, m, color)
                return 1
        self.store(code, -1, None, color)
        return -1 

    # initial call
    def callNegamax(self):
        b = self.copy()
        result = b.negamax(self.current_player)
        self.table = b.table
        win_move = self.lookup(self.code(), self.current_player)[1]
