...
class UCB:
    ...
    def compute_ucb(self, num, val, N,anum, aval):
        '''
        calculate the upper confidence bound
        '''
        q = val/num
        rave = aval / anum
        beta = np.sqrt(150 / (3 * N + 150))
    ...
    def select(self, stats, N, moves):
        ...
        for index, (num, val) in enumerate(stats):
            ...
            anum, aval = self.amaf[move]
            ucb = self.compute_ucb(num, val, N, anum, aval)
            if ucb > max_val:
                max_val = ucb
                max_index = index
    ...
    def run_ucb(self, board:GoBoard, moves, color):
        ...
        for N in range(1, total_sim+1):
            ...
            index = self.select(stats, N, moves)
            move = moves[index]
    ...
    def play_game(self, board:GoBoard):
        trace = []
        while True:
